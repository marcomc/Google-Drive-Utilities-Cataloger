...
run_apps_script_function() {
  ...
  if [[ ! "${deployment_id}" =~ ^[A-Za-z0-9_-]+$ ||
    ! "${function_name}" =~ ^[A-Za-z_$][A-Za-z0-9_$]*$ ]]; then
    printf '%s\n' "Invalid Apps Script execution target." >&2
    return 1
  fi
  if ! "${CLASP[@]}" -A "${auth_file}" --json deployments >/dev/null; then
    printf '%s\n' \
      "Could not refresh authorization for Apps Script execution." >&2
    return 1
  fi
  access_token="$(jq -er '
    .tokens.default.access_token |
  ...
  request_json="$(jq -cn --arg function_name "${function_name}" '
    {function: $function_name, parameters: [], devMode: false}
  ')"
  execution_url="https://script.googleapis.com/v1/scripts/${deployment_id}:run"
  if http_response="$(
  ...
  unset access_token
  if [[ "${curl_status}" -ne 0 ]]; then
    printf 'Apps Script execution transport failed with curl status %s.\n' \
      "${curl_status}" >&2
    return 1
  fi
  if [[ "${http_response}" != *$'\n'* ]]; then
    printf '%s\n' "The Apps Script Execution API returned no HTTP status." >&2
    return 1
  fi
  http_status="${http_response##*$'\n'}"
  response_json="${http_response%$'\n'*}"
  if [[ "${http_status}" != "200" ]]; then
    printf 'Apps Script execution failed with HTTP %s.\n' \
      "${http_status}" >&2
    return 1
  fi
  if ! jq -e 'type == "object"' <<<"${response_json}" >/dev/null; then
    printf '%s\n' "The Apps Script Execution API returned invalid JSON." >&2
    return 1
  fi
  printf -v "${result_variable}" '%s' "${response_json}"
}
